...
Calls `generate_image`. Flags map directly onto the tool's parameters: `<prompt>` → `prompt`, `--model` → `model` (default `gpt-image-2` if omitted), `--ar` → `aspectRatio` (default `1:1`), `--res` → `resolution` (default `1K` on models that support tiers), `--ref` → `referenceImageUrls` (repeat the flag up to 4 times for multiple references). Omit any flag you don't have a strong reason to set — the defaults are sensible.
...
Call `generate_image` once per size in `--sizes` (default `1:1,9:16,16:9`), reusing the same `<concept>` and the same style wording — subject, lighting, medium, mood — in every prompt. Only the framing/composition should change between sizes, never the subject or style, so the set reads as one campaign. Confirm before running more than 4 sizes in one go. Return a plain size → URL list when done:
...
Before calling anything, state the estimated cost using the price table in section 4 (or a fresh `list_models` call) for the model/duration you're about to use. Then call `generate_video` (`<prompt>` → `prompt`, `--from` → `startImageUrl`, `--model` → `model`, default `kling-2-1-std`, `--sec` → `duration`, default `5`, `--ar` → `aspectRatio`, default `16:9`). It only *starts* the render and repeats its own cost estimate in the response — tell the user it takes 2–5 minutes. Only call `check_status` if the user asks, and only once, after waiting at least 2–3 minutes.

### `/animate <imageUrl> <motion description>`

Same tool as `/video`, but `<imageUrl>` is required and maps to `startImageUrl` so that exact image is animated instead of an auto-generated start frame; `<motion description>` maps to `prompt` — describe the motion and camera, not the subject, since the image already shows the subject. Same cost-first, no-loop rules as `/video`.
...
Call `check_credits`. Report the balance in credits and USD. If it looks too thin for the next likely request (roughly under $1 for an image, under $2 for a video), say so and suggest topping up at kie.ai.

### `/models`

Call `list_models`, then summarize using the tables in section 4 rather than dumping the raw output.
