---
name: grok-imagine
description: Long Cheap Video Generator (Grok Imagine) — FreshGen video model skill. Render a short video with the Grok Imagine model (`grok-imagine`), best for longer cheap clips (6–10s). Use when the user types /grok-imagine, names this model, or the request clearly fits it. Real money (about $0.30 / 6s), billed to the connected Kie.ai account; quote the cost before starting.
---

# Long Cheap Video Generator (Grok Imagine)

**Long Cheap Video Generator** — the FreshGen skill for the Grok Imagine video model, best for longer cheap clips (6–10s). Slash command: `/grok-imagine`.

## The `/grok-imagine` command

Signature: `/grok-imagine <prompt> [--from <imageUrl>] [--sec 6 / 10] [--res 480p / 720p] [--ar 16:9 / 9:16 / 1:1]`

When a message begins with this command (or names this model): first state the estimated cost for the chosen duration and resolution (see below), then call `generate_video` with `model: "grok-imagine"` and `prompt` = the user's prompt. Map flags: `--from` → `startImageUrl` (animate that exact image; omit to auto-generate a start frame, which adds ~$0.04 and ~30s), `--sec` → `duration` (6 or 10; default `6`), `--res` → `resolution` (480p/720p), `--ar` → `aspectRatio` (default `16:9`). The tool only STARTS the render and returns a task id — tell the user it takes 2–5 minutes, and only call `check_status` once, after 2–3 minutes, if they ask. Finished clips auto-save to the GHL Media Library when that is configured.
...
| Best for | longer cheap clips (6–10s) |
| Approx. cost | ≈ $0.30 / 6s |
| Durations | 6, 10 seconds (default 6) |
