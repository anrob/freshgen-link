...
When a message begins with this command (or names this model), call `generate_image` with `model: "gpt-image-2"` and `prompt` = the user's prompt. Map flags to parameters: `--ar` → `aspectRatio` (default `1:1`), `--res` → `resolution` (1K/2K/4K; default `1K`), `--ref` → `referenceImageUrls` (repeat up to 4 times). Prepend the BRAND block style unless the user says otherwise.
