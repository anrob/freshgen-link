---
name: nano-banana-2
description: Fast All-Round Image Generator (Nano Banana 2) — FreshGen image model skill. Generate an image with the Nano Banana 2 model (`nano-banana-2`), best for fast all-round quality at a low price. Use when the user types /nano-banana-2, names this model, or the request clearly fits it. Real money (about $0.04 / image (1K)), billed to the connected Kie.ai account.
---

# Fast All-Round Image Generator (Nano Banana 2)

**Fast All-Round Image Generator** — the FreshGen skill for the Nano Banana 2 model, best for fast all-round quality at a low price. Slash command: `/nano-banana-2`.

## The `/nano-banana-2` command

Signature: `/nano-banana-2 <prompt> [--ar 16:9 / 1:1 / 9:16 / 4:5]`

When a message begins with this command (or names this model), call `generate_image` with `model: "nano-banana-2"` and `prompt` = the user's prompt. Map flags to parameters: `--ar` → `aspectRatio` (default `1:1`). This model does NOT accept reference images — if the user provides one, say so and suggest `nano-banana-pro` or `gpt-image-2` instead. This model renders at a fixed resolution — there is no `--res` flag. Prepend the BRAND block style unless the user says otherwise.
...
| Best for | fast all-round quality at a low price |
| Approx. cost | ≈ $0.04 / image (1K) |
| Resolutions | fixed |
| Reference images | no |

## Prompting tips for Nano Banana 2

- Fast, low-cost, good all-rounder — the sensible default when the request has no text in the image and no reference photo.
